...
public class Human {
	...
	@Watch(watcheeClassName = "jzombie.Zombie", watcheeFieldNames = "moved", query = "within_moore 1", whenToTrigger = WatcherTriggerSchedule.IMMEDIATE)
	public void run() {
		...
		GridPoint pointWithLeastZombie = null;
		int minCount = -1;
		for (GridCell<Zombie> cell : gridCells) {
			if (cell.size() > minCount) {
				pointWithLeastZombie = cell.getPoint();
				minCount = cell.size();
			}
		}
		
		if(energy > 0 ) {
			moveTowards(pointWithLeastZombie);
		} else {
